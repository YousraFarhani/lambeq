...
class ArabicParser2(CCGParser):
    ...
    def convert_to_ccg(self, atb_tree: list[dict]) -> CCGTree:
        ...
        # Adjust for Verb-Subject-Object (VSO) word order
        nodes = sorted(nodes, key=lambda x: x.rule == CCGRule.LEXICAL and x.biclosed_type == CCGType.SENTENCE, reverse=True)
    ...
    def map_pos_to_ccg(self, atb_pos: str, dependency: str) -> CCGType:
        ...
        atb_to_ccg_map = {
            ...
            "JJ": CCGType.NOUN.slash("\\", CCGType.NOUN),  # Adjusted for post-nominal adjectives
        ...
        if dependency in ["amod", "acl"]:
            return CCGType.NOUN.slash("\\", CCGType.NOUN)  # Adjusted for Arabic adjective order
